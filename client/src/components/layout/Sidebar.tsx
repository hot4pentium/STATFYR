...
import { Shield, LogOut, User, Trophy, Calendar, Home, MessageSquare, ClipboardList, Users } from "lucide-react";
  ...
  return (
    <div className="h-screen w-64 bg-sidebar border-r border-sidebar-border flex flex-col fixed left-0 top-0 z-50">
      <div className="p-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-xl font-bold tracking-tight text-sidebar-primary-foreground">
            TEAMPULSE
          </h1>
          <p className="text-xs text-sidebar-foreground uppercase tracking-wider">{TEAM_NAME}</p>
        </div>
      </div>
        ...
        {links.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href}>
              ...
              >
                <item.icon className={cn("h-5 w-5", isActive ? "text-sidebar-primary" : "text-sidebar-foreground/70 group-hover:text-sidebar-accent-foreground")} />
                {item.name}
              </div>
            </Link>
          );
      ...
      <div className="p-4 border-t border-sidebar-border">
        ...
        <Link href="/">
          <Button variant="outline" className="w-full justify-start text-sidebar-foreground hover:text-sidebar-accent-foreground border-sidebar-border hover:bg-sidebar-accent">
